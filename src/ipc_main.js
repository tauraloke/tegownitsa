...
ipcMain.handle('executeQueryAll', async (_event, query, values) => {
  return await db.queryAll(query, values);
});

ipcMain.handle('openFolder', async (_event) => {
...
ipcMain.handle('findTagsByFile', async (_event, file_id) => {
...
ipcMain.handle(
  'addTag',
  async (_event, file_id, title, locale, source_type) => {
    title = title.trim();
    let tag_id = null;
