...
export async function run(_event, db, file_id) {
  ...
  return db.queryAll(
    `SELECT 
      files.*,
      files2.id AS f2_id,
      files2.preview_path AS f2_preview_path,
      files2.width AS f2_width,
      files2.height AS f2_height,
      files2.source_filename AS f2_source_filename,
      hamming(files.imagehash, files2.imagehash) AS distance
    FROM files
    JOIN files AS files2 ON files.id > files2.id
    WHERE distance < ? AND files.id=?`,
