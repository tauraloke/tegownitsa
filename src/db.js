...
async function initDatabase({ dbPath }) {
	...
	await dbConnection.run(
		"CREATE TABLE IF NOT EXISTS tag_locale (id integer PRIMARY KEY AUTOINCREMENT NOT NULL, tag_id INTEGER, title TEXT, locale VARCHAR(2), created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP)"
	);
	await dbConnection.run(
